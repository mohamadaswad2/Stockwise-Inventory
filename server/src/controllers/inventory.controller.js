...
const restockItem    = async (req,res,next) => { try { success(res, await inventoryService.restockItem(req.user.id, req.params.id, req.body), 'Restock recorded.'); } catch(e){next(e);} };
const exportCSV      = async (req,res,next) => {
  try {
    const csv = await inventoryService.exportCSV(req.user.id, req.user.plan);
    res.setHeader('Content-Type', 'text/csv; charset=utf-8');
    res.setHeader('Content-Disposition', `attachment; filename="stockwise-${new Date().toISOString().slice(0,10)}.csv"`);
    res.send('\uFEFF' + csv);
  } catch(e){next(e);}
};
const getExportQuota = async (req,res,next) => { try { success(res, await inventoryService.getExportQuota(req.user.id, req.user.plan)); } catch(e){next(e);} };
