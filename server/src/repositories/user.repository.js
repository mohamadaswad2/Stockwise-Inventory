...
const create = async ({ name, email, password, email_verify_token, email_verify_expires, trialEndsAt, plan }) => {
  const result = await db.query(
    ...
    [name, email.toLowerCase(), password, email_verify_token, email_verify_expires, trialEndsAt, plan || 'deluxe', false]
  );
