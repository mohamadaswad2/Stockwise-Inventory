...
function ChartTooltip({ active, payload, label }) {
  ...
  return (
    <div className="card px-3 py-2 text-xs">
      ...
      {payload.map(p => <p key={p.name} style={{ color: p.color }}>{p.name}: <strong>RM {Number(p.value).toFixed(2)}</strong></p>)}
    </div>
  );
}
        ...
        {loading ? (
          ...
          <div className="space-y-5 animate-ios-in">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              <SaleCard icon={DollarSign}  label="Total Revenue"   value={`RM ${Number(data?.sales?.total_revenue||0).toFixed(2)}`} color="green" />
              <SaleCard icon={TrendingUp}  label="Last 30 Days"    value={`RM ${Number(data?.sales?.revenue_30d||0).toFixed(2)}`}   color="blue" />
            ...
            <div className="card p-5">
              <h3 className="text-sm font-bold mb-4" style={{ color: 'var(--text)' }}>Revenue — Last 30 Days</h3>
              {data?.trend?.length > 0 ? (
                <ResponsiveContainer width="100%" height={200}>
                  <AreaChart data={data.trend} margin={{ top: 4, right: 4, left: -15, bottom: 0 }}>
                    ...
                    <Area type="monotone" dataKey="revenue" name="Revenue"
                      stroke="#22c55e" strokeWidth={2} fill="url(#rev)" />
                  </AreaChart>
                </ResponsiveContainer>
              ) : (
