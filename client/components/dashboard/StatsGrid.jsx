...
export default function StatsGrid({ stats, loading }) {
  ...
  return (
    <div className="grid grid-cols-2 xl:grid-cols-4 gap-3">
      ...
      <StatCard icon={Layers}        label="Total Stock"     value={stats?.total_quantity}             color="green"  sub="units in inventory" />
      ...
      <StatCard icon={DollarSign}    label="Inventory Value" value={`RM ${stats?.total_value ?? '0'}`} color="purple" />
    </div>
  );
}
